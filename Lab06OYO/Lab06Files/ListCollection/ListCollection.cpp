...
{
    ListCollection newList((numElements-elem)+1);
    newList.numElements = numElements-elem+1;
    newList.numCapacity = newList.numElements;
    for(int i=0; i<newList.numElements; i++)
    {
        newList.list[i] = list[i+elem];
    }
...
{
    numElements+=1; // since an element is being added to the list, the number of elements will increase by 1
...
//     }
//     int temp = this->numCapacity;
//     pushFront(elem);
//    // *this = this->sort();
//     this->numCapacity = temp;
